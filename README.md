...
GitHub Releases 的原生下载链路在部分地区偏慢，因此本站把最新 APK **镜像到本仓库并由 GitHub Pages（Fastly）同源分发**：

- `update-release-info.yml` 每小时从 app 仓库拉取最新 Release 的 APK，下载到本仓库 `apk/` 并提交；
- 下载按钮使用 GitHub Pages 地址（`https://aasdqwe1.github.io/agent-toolbox-site/apk/agent-toolbox-latest.apk`，Fastly CDN，同源、免重定向），随每次 CI 构建自动更新；
- 若 GitHub Pages 镜像不可用，自动回退到 GitHub Releases 原链接。
